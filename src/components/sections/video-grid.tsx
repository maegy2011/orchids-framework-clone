    ...
    <div className="w-full px-4 sm:px-6 py-6 min-h-[400px]" dir={direction}>
                        ...
                        <div className="flex items-center gap-1 hover:text-foreground transition-colors">
                          ...
                          {video.isVerified && <CheckCircle2 size={14} className="text-muted-foreground fill-muted-foreground/10" />}
                            ...
                            <div className="flex items-center gap-1">
                              <span>{video.views.toLowerCase().includes('view') || video.views.includes('مشاهدة') || video.views.includes(t('views')) ? video.views : `${video.views} ${t('views')}`}</span>
                              <span className="before:content-['•'] before:mx-1">{video.uploadedAt || t('unknown')}</span>
                            </div>
      ...
      {!isLoading && videos.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
          ...
          <p className="text-sm">{t('try_different_keywords')}</p>
        </div>
      )}
    </div>
  );

  ...
  return (
    <>
      <AnimatePresence>
        ...
        <header className={`fixed ${headerTop} left-0 right-0 z-[8000] flex items-center justify-between h-[64px] px-4 bg-background/80 backdrop-blur-md border-b border-border select-none transition-all duration-300`}>
            ...
            <Link href="/" onClick={handleClearState} className="flex items-center group px-1">
              <div className="flex items-center">
                <div className="relative flex items-center justify-center w-20 h-20 -my-4">
                  <div className="absolute inset-4 bg-red-100 dark:bg-red-900/30 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-500 scale-75 group-hover:scale-100 blur-md" />
                  <OrchidIcon
                    className={cn(
                      "relative w-20 h-20 transition-all duration-500 drop-shadow-2xl",
                      theme === 'boys' ? 'text-sky-500' : theme === 'girls' ? 'text-pink-500' : 'text-red-600'
                    )}
                  />
                </div>
                <span className={cn(
                  ...
                  {suggestions.map((suggestion, index) => (
                    <button
                      ...
                      onMouseEnter={() => setSelectedIndex(index)}
                        className={`w-full px-5 py-2.5 hover:bg-muted flex items-center gap-3 text-[15px] font-medium text-foreground transition-colors ${selectedIndex === index ? 'bg-muted' : ''} flex-row`}
                        dir={direction}
                    >
